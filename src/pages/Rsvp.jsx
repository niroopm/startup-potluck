...
export default function Rsvp() {
  ...
  async function handleSubmit() {
    if (!member || selected.isNoEvent) return;
    setSubmitting(true);
    const sessionTag = selected.isOutdoor
      ? 'Outdoor Free Session (5:00 PM)'
      : (attend === 'yes' ? 'Closed-Door Pass ₹150 (10:30 AM)' : 'Declined Attendance');
    try {
      await sheetPost({
        type: 'rsvp',
        name: member.name,
      ...
      <div className="max" style={{ maxWidth: 780 }}>
          ...
          {sundays.map((s, i) => (
            <button
              ...
              style={{
                ...
                padding: 16,
                opacity: s.isNoEvent ? 0.7 : 1
              }}
            >
              <div style={{
                fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, marginBottom: 4,
                color: s.isNoEvent ? 'var(--ink-faint)' : (s.isOutdoor ? 'var(--green)' : 'var(--purple)')
              }}>
                {s.isNoEvent ? 'NO EVENT' : (s.isOutdoor ? 'OUTDOOR · FREE' : 'CLOSED-DOOR · ₹150')}
              </div>
              <div style={{ fontWeight: 600, fontSize: 15 }}>{s.shortDate}</div>
              <div style={{ fontSize: 11.5, color: 'var(--ink-soft)' }}>
                {getOrdinal(s.weekRank)} Sunday{!s.isNoEvent && <> · {s.isOutdoor ? '5:00 PM' : '10:30 AM'}</>}
              </div>
            </button>
          ))}
        </div>
        ...
        {selected.isNoEvent ? (
          <div className="flow-card" style={{ textAlign: 'center', maxWidth: 420, margin: '0 auto' }}>
            ...
            <h3 style={{ fontSize: 16, marginBottom: 8 }}>No Event This Sunday</h3>
            <p style={{ fontSize: 12.5, color: 'var(--ink-soft)', marginBottom: 4 }}>{selected.dateString}</p>
            <p style={{ fontSize: 12.5, color: 'var(--ink-soft)' }}>
              This month has a 5th Sunday, so the table takes a break. No RSVP needed — see you the next 1st or 2nd Sunday!
            </p>
          </div>
        ) : !done ? (
          <div className="flow-card">
            ...
            <div className={`session-banner${selected.isOutdoor ? ' outdoor-banner' : ''}`}>
              <div className="session-banner-title">
                <span>{selected.isOutdoor ? '🌳 Outdoor Park Session' : '🚪 Closed-Door Session'} ({getOrdinal(selected.weekRank)} Sunday)</span>
          ...
          <div className="flow-card" style={{ textAlign: 'center', maxWidth: 420, margin: '0 auto' }}>
            ...
            <div style={{ background: 'var(--paper)', padding: 12, borderRadius: 8, border: '1px solid var(--rule)', textAlign: 'left', fontSize: 12.5 }}>
              <p style={{ marginBottom: 4 }}><span style={{ color: 'var(--ink-faint)' }}>Session:</span> <strong>{selected.isOutdoor ? 'Outdoor Park (5:00 PM)' : 'Closed-Door (10:30 AM)'}</strong></p>
