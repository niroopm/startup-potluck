...
import { getUpcomingSundayDetails } from '../lib/calendar.js';
...
const PARTNERS = [
  {
    ribbon: 'TECH PARTNER',
    ribbonColor: 'var(--purple-deep)',
    avatarBg: 'linear-gradient(135deg,#1C1A16,#4B21D6)',
    initials: 'HL',
    name: 'HelloLocal',
  ...
  {
    ribbon: 'DIGITAL PARTNER',
    ribbonColor: 'var(--green)',
    avatarBg: 'linear-gradient(135deg,#1E8A4C,#6C3BFF)',
    initials: 'AD',
    name: 'ARANEA DEN',
  ...
  return (
    <>
      ...
      <section className="hero" style={{ padding: '64px 0 48px' }}>
        <div className="max hero-grid" style={{ alignItems: 'center', gap: 48 }}>
          <div>
            ...
            <p className="lede" style={{ margin: '20px 0 32px', fontSize: 17, lineHeight: 1.6 }}>
            ...
            <div className="hero-ctas" style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              ...
              {!info.isNoEvent && <Link to="/rsvp" className="btn btn-ghost">RSVP This Sunday</Link>}
            </div>
          </div>

          <div className="ticket-wrap" style={{ display: 'flex', justifyContent: 'center' }}>
            {info.isNoEvent ? (
              <div className="ticket no-event-ticket">
                <div className="ticket-main">
                  <div className="t-eyebrow">5TH SUNDAY</div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
                    <h3 style={{ margin: 0 }}>No Event<br/>This Sunday</h3>
                    <div className="ticket-icon-box">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#8C876F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <circle cx="12" cy="12" r="9"/><path d="M9 9l6 6M15 9l-6 6"/>
                      </svg>
                    </div>
                  </div>
                  <p style={{ fontSize: 13, color: 'var(--ink-soft)', marginTop: 12, lineHeight: 1.5 }}>
                    Months with a 5th Sunday take a break — see you the next 1st or 2nd Sunday!
                  </p>
                  <div className="ticket-fields" style={{ marginTop: 16 }}>
                    <div><span>DATE</span><strong>{info.dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</strong></div>
                    <div><span>STATUS</span><strong>No Session</strong></div>
                  </div>
                </div>
                <div className="ticket-divider"></div>
                <div className="ticket-stub">
                  <span className="stub-no">№ 001</span>
                  <span className="stub-vert">SKIP →</span>
                </div>
              </div>
            ) : (
      ...
      <section style={{ padding: '0 0 64px' }}>
        <div className="max">
          <div className="flow-head" style={{ textAlign: 'center', marginBottom: 36 }}>
            <div className="eyebrow center"><span className="dot"></span>Ritual Flow</div>
            <h2 className="section-title">How the Sunday Works</h2>
            <p className="section-lede center" style={{ marginTop: 8 }}>A simple weekly cycle built to turn conversations into long-term ventures.</p>
          </div>

          <div className="grid cols-3" style={{ gap: 16 }}>
            {HOW_STEPS.map(s => (
              <div key={s.n} className="pin-card card" style={{ textAlign: 'center', padding: '22px 16px' }}>
                <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--purple)', fontSize: 12, fontWeight: 700, marginBottom: 8 }}>{s.n}</div>
                <p style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>{s.t}</p>
                <p style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.4 }}>{s.d}</p>
              </div>
            ))}
          </div>

          <div style={{ marginTop: 32, border: '1.5px dashed var(--rule)', borderRadius: 14, padding: '24px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            <div>
              <p style={{ fontWeight: 600, fontSize: 15, color: 'var(--ink)' }}>Let's build something amazing together.</p>
              <p style={{ fontSize: 13, color: 'var(--ink-soft)', marginTop: 2 }}>Good food. Great people. Bigger ideas.</p>
            </div>
            <Link to="/rsvp" className="btn btn-accent">RSVP This Sunday</Link>
          </div>
        </div>
      </section>

      {/* ============================= RULES & ETIQUETTE ============================= */}
      <section id="rules" style={{ padding: '0 0 64px' }}>
        <div className="max">
          <div className="flow-head" style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <div style={{ marginBottom: 36 }}>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--purple)', marginBottom: 14, fontWeight: 600 }}>
              📍 Meeting Room Etiquette
            </p>
            <div className="card kraft" style={{ padding: 16 }}>
              <div className="grid cols-3" style={{ gap: 12 }}>
                {MEETING_RULES.map((r, i) => (
                  <div key={r} className="card" style={{ padding: '14px 16px', borderColor: 'var(--rule)', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--purple)', fontWeight: 700, fontSize: 12 }}>{String(i + 1).padStart(2, '0')}</span>
                    <h4 style={{ fontSize: 13.5, fontWeight: 600, margin: 0 }}>{r}</h4>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ marginBottom: 36 }}>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--purple)', marginBottom: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {/* ============================= STRATEGIC ROADMAP (MOVED HERE) ============================= */}
          <div className="card" style={{ padding: 28, background: '#fff', border: '1.5px solid var(--ink)' }}>
            <div style={{ marginBottom: 16 }}>
              ...
              <h3 style={{ fontSize: 20, color: 'var(--ink)', margin: 0 }}>🎯 5 Core Objectives</h3>
            </div>
            ...
            <div className="grid cols-2" style={{ gap: 12 }}>
              {CORE_OBJECTIVES.map(o => (
                <div key={o.t} style={{ background: 'var(--paper)', padding: 14, borderRadius: 10, border: '1px solid var(--rule)' }}>
                  ...
                  <p style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.4, margin: 0 }}>{o.d}</p>
                </div>
              ))}
            </div>
            <div style={{ background: 'var(--paper)', padding: 14, borderRadius: 10, border: '1px solid var(--rule)', marginTop: 12 }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.4, margin: 0 }}>Solve Godavari-regional challenges (agritech, tourism, retail) and create local jobs.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ============================= ABOUT FOUNDER ============================= */}
      <section id="about" className="compact-section" style={{ background: 'var(--paper-deep)', padding: '64px 0' }}>
        <div className="max" style={{ maxWidth: 880 }}>
          <div className="flow-head" style={{ textAlign: 'left', marginBottom: 24 }}>
          ...
          <div className="card" style={{ padding: 32, background: '#fff', border: '1.5px solid var(--ink)' }}>
            <div style={{ display: 'flex', gap: 32, alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ flexShrink: 0, textAlign: 'center', width: '100%', maxWidth: 210, margin: '0 auto' }}>
                ...
                <p style={{ fontSize: 11, color: 'var(--purple)', fontFamily: 'var(--font-mono)', fontWeight: 600, textTransform: 'uppercase', marginTop: 2, marginBottom: 12 }}>Founder & Community Lead</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginTop: 14 }}>
                  <a href="https://www.instagram.com/dr.joharkrishna?utm_source=qr&igsh=dHM1bXM1d2VuOWZt" target="_blank" rel="noopener noreferrer" title="Instagram">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#6C3BFF" strokeWidth="2">
      ...
      {/* ============================= PARTNERS ============================= */}
      <section id="partners" className="compact-section" style={{ background: 'var(--paper-deep)', textAlign: 'center', padding: '0 0 64px' }}>
        <div className="max">
          <div className="flow-head" style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 20, flexWrap: 'wrap' }}>
            {PARTNERS.map(p => (
              <div key={p.name} className="badge" style={{ maxWidth: 320, width: '100%' }}>
                ...
                <p style={{ fontWeight: 600, fontSize: 15, margin: 0 }}>{p.name}</p>
                <p style={{ fontSize: 12.5, color: 'var(--ink-soft)', marginTop: 2 }}>{p.role}</p>
                <p style={{ fontSize: 11.5, color: 'var(--ink-faint)', fontStyle: 'italic', marginTop: 12, paddingTop: 10, borderTop: '1px dotted var(--rule)', textAlign: 'center', lineHeight: 1.5 }}>
                  {p.blurb}
                </p>
